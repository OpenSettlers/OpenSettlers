 ...
 */
public class BoardPickerBitmapWidget extends Composite implements BoardPickerWidget
{
  private EventBus eventBus = new SimpleEventBus();
  private HorizontalPanel rootPanel = new HorizontalPanel();
  private BoardProvider boardProvider = new ConstructorBoardProvider();
  private CellTable<Board> cellTable;
  private List<Board> boards = boardProvider.all();
  ...
  {
    @Override public Object getKey(Board item)
    {
  ...
  public BoardPickerBitmapWidget()
  {
    ...
    {
      @Override public Number getValue(Board object)
      {
        return object.getBoardSettings().getVpToWin().vpToWin();
      }
    };
    cellTable.addColumn(columnVP, "Points");
    TextColumn designerColumn = new TextColumn<Board>()
    {
      @Override public String getValue(Board object)
      {
  ...
  @Override public HandlerRegistration addBoardChangedHandler(
          BoardChangedHandler handler)
  {
    return eventBus.addHandler(BoardChangedEvent.getType(), handler);
  }
  @Override public Board selectedBoard()
  {
    return selectedBoard;
  }
  @Override public boolean hasBoardSelected()
  {
    return selectedBoard != null;
  }
  @Override public void selectFirst()
  {
    selectionModel.setSelected(boardProvider.all().get(0), true);
  }
}