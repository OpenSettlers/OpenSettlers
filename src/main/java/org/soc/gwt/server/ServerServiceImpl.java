        ...
        ServerService
{
  ...
  @Override public User login(String nickName, String password)
  {
    ...
    httpSession.setAttribute("playername", user.name());
  ...
  @Override public void logout(User user)
  {
    ...
    if (!user.name().equals(httpSession.getAttribute("playername")))
    {
      // throw new ChatException("User: " + username +
  ...
  @Override public void send(LobbyAction action)
  {
    // check if there is a HTTP session setup.
    HttpSession httpSession = getThreadLocalRequest().getSession(false);
    if (httpSession == null)
