...
import org.soc.common.game.board.Route.PossibleRoute;

/** A PlayerGraph is a graph containing all edges (roads, ships, bridges) and vertices (towns,
 * cities, knights) of a player. The PlayerGraph does not contain edges of opponents, but it can
 * contain vertices (cities, towns, knights) from opponents. */
public class PlayerGraph {
  ...
  /** Returns the other point for a given 2-degree middle GraphPoint (middle), and given originating
   * GraphPoint (from) */
  ...
  /** Returns a list of GraphPoints given a middle 3-degree GraphPoint and a given originating
   * GraphPoint (from) */
   ...
   * 
   * 1. Creates a list of routes assembled from ends and splits. 2. Loops until all routes
   * encountered an end. 3. Each iteration takes a route, and either detects the route ends, or adds
   * the lookahead GraphPoint. 4. Loop ends when all routes have ended.
   * 
   * Takes ends and splits as starting points to calculate the longest path. This will usually work,
   * but not when there is a loop without an end or split. In that case, the loopChecker will detect
   * the loop and add another possible route.
   * 
   * Loops without ends and splits may occur multiple times, and more then 2 times when a volcano is
   * supported (extremely rare edge case, but still possible). Sequences of roads may be split when
   * a volcano disconnects a road + town + ship combination, and the ship is later moved away. */
  ...
  /** Creates a list of routes to start a walk with on the player graph. These will be ends and
   * splits, loops without ends and splits are therefore not checked when walking the graph using
   * only the ends and splits. */
  private void createStartRoutes(List<PossibleRoute> routesToProcess) {
    ...
    for (GraphPoint end : ends)
      routesToProcess.add(new PossibleRoute(boardGraph, end, otherPoint(end)));
    // Create a set of routes for every split
    for (GraphPoint split : splits)
      for (GraphSide side : boardGraph.graph().edgesOf(split)) {
  ...
  /** Assembles a list of single-connected points, and a list of tri-connected points */
