 ...
 *<P>
 * Text announcements ({@link GameTextMsg}) are often sent along with data
 * messages.
 *<P>
 * The message data is sent over the network as type ID + data strings built by
 * each Message subclass's toCmd() method.
 *<P>
 * On the remote end, it's reconstructed to a new instance of the appropriate
 * Message subclass, by the subclass' required method static MessageSubclass
 * parseDataStr(String). parseDataStr is called from {@link #toMsg(String)} in
 * this class.
 *<P>
 * The client receives messages in
 * {@link soc.client.PlayerClient#treat(Message, boolean)}. The server receives
 * messages in
 * {@link soc.server.SOCServer#processCommand(String, StringConnection)}.
 *<P>
 * To create and add a new message type:
 *<OL>
 * <LI>Decide the message type name. Add to the end of the constant list in this
 * class. Add a comment to note the OpenSettlers version in which it was
 * introduced, and the date.
 * <LI>If the new message is for something that any kind of game can use, give
 * it the next available type ID number in the list (10xx). If the message is
 * specific to the OpenSettlers game and its interface, use a message number
 ...
 * parent-classes can help; the example subclasses extend them. Be sure to
 * override the minimum version reported in {@link #getMinimumVersion()}.
 * <LI>Add to the switch in PlayerClient.treat and/or SOCServer.processCommand.
 * Note the OpenSettlers version with a comment.
 * <P>
 * <em>Note:</em> Most things added to PlayerClient.treat should also be added
 * to {@link soc.client.DisplaylessPlayerClient#treat(Message)}, to
 * {@link soc.robot.RobotClient#treat(Message)}, and possibly to
 * {@link soc.robot.RobotBrain#run()}.
 *</OL>
 *<P>
 * Backwards compatability: Unknown message types are ignored by client and by
 * server. Technically they are returned as null from {@link #toMsg(String)} if
 * the local copy of Message doesn't know that message type.
 *<P>
 * Format: For most messages, at most one {@link #sep} token per message, which
 * separates the messagetype number from the message data; multiple
 * {@link #sep2} are allowed after {@link #sep}. For multi-messages, multiple
 * {@link #sep} are allowed; see {@link MessageMulti}.
 * 
...
public abstract class Message implements Serializable, Cloneable
{
    ...
    /**
     * To identify new message types, give the minimum version where this type
     * is used. Default of 1000 (version 1.0.00) unless overridden.
     * 
     ...
     * type is used. Default (for active messages) returns
     * {@link Integer#MAX_VALUE}.
     * 
     * @return Version number, as in 1006 for OpenSettlers 1.0.06, or
     *         {@link Integer#MAX_VALUE}.
     */
    ...
    /**
     * Converts the contents of this message into a String that can be
     * transferred by a client or server. Your class' required method static
     * SOCMessageSubclass parseDataStr(String) must be able to turn this String
     * back into an instance of the message class.
     *<P>
     * For most message types, at most one {@link #sep} token is allowed,
     * separating the type ID from the rest of the parameters. For
     * multi-messages (@link MessageMulti}, multiple {@link #sep} tokens are
     * allowed. Multi-messages are parsed with: static SOCMessageSubclass
     * parseDataStr(String[])
     */
    ...
    /**
     * For use in toString: Append int array contents to stringbuffer, formatted
     * as "{ 1 2 3 4 5 }".
     * 
     * @param ia
     *            int array to append. 0 length is allowed, null is not.
     * @param sb
     *            StringBuffer to which <tt>ia</tt> will be appended, as
     *            "{ 1 2 3 4 5 }"
     ...
     * 
     * @param sv
     *            Enum of String to append. 0 length is allowed, null is not
     *            allowed.
     * @param sb
     *            StringBuffer to which <tt>se</tt> will be appended, as
     *            "a,b,c,d,e"
     * @throws ClassCastException
     *             if <tt>se.nextElement()</tt> returns non-String
     * @throws NullPointerException
    ...
    /**
     * Test whether a string is non-empty and its characters are all 'safe' as a
     * single-line string: No newlines or {@link Character#isISOControl(char)
     * control characters}, no {@link Character#isSpaceChar(char) line
     * separators or paragraph separators}. Whitespace character type
     * {@link Character#SPACE_SEPARATOR} is OK. Must not contain
     * {@link #sep_char} or {@link #sep2_char}.
     * 
     * @param s
     *            string to test; if null, returns false.
     * @return true if all characters are OK, false otherwise. Null string or
     *         0-length string returns false.
     */
    ...
    /**
     * Utility, place one string into a new single-element array. To assist with
     * {@link MessageMulti} parsing.
     * 
     * @param s
     *            String to place into array, or null
    ...
    /**
     * Convert a string into a Message. The string is in the form of
     * "id SEP messagename {SEP2 messagedata}*". If the message type id is
     * unknown, this is printed to System.err.
     * 
     * @param s
     *            String to convert
    ...
    public static Message toMsg(String s)
    {
        try
        {
            ...
            /**
             * to handle {@link MessageMulti} subclasses - multiple parameters
             * with sub-fields. If only one param is seen, this will be null;
             * use {@link #toSingleElemArray(String)} to build it.
             *<P>
             * Note that if you passed a non-null gamename to the
             * {@link MessageTemplateMs} or {@link MessageTemplateMi}
             * constructor, then multiData[0] here will be gamename, and
             * multiData[1] == param[0] as passed to that constructor. <code>
