...
public class RollDice extends AbstractTurnAction
{
    ...
    public void perform(Game game)
    {
        ...
        if (dice.getDice() != 7)
        {
            ...
            for (AbstractHex hex : game.getBoard().getHexes())
            {
            ...
            for (ResourceHex hex : rolledHexes)
            {
                ...
                if (!hex.getLocation().equals(robber))
                {
                    for (Player player1 : game.getPlayers())
                    {
                        ...
                        for (PlayerPiece piece : player1.getBuildPieces().ofType(City.CITY))
                        {
                            cities.add((City)piece);
                        }
                        
                        List<Town> towns = new ArrayList<Town>();
                        for (PlayerPiece piece1 : player1.getBuildPieces().ofType(Town.TOWN))
                        {
            ...
            {
                ResourceList gainedResources = playersResources.get(resourcePlayer);
                if (gainedResources.size() > 0)
                {
                    resourcePlayer.addResources(gainedResources);
                    game.getBank().remove(gainedResources, false);
                    message += resourcePlayer.getName() + " gained " + gainedResources.toString();
                }
            }
        ...
        else
        {
            ...
            {
                if (p1.getResources().size() > p1.getMaximumCardsInHandWhenSeven())
                {
            ...
            {
                message = message + playerList + " loose half of their resources";
            }
            else
            {
