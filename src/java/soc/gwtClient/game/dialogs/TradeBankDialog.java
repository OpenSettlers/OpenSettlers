...
public class TradeBankDialog extends DialogBox
{
    ...
    public TradeBankDialog(IGamePanel gamePanel)
    {
        ...
        giveResourcesListWidget = createResourceListWidget(
                giveResources, playerHand, gamePanel.getPlayingPlayer().getPorts());
        
        wantedResourcesListWidget = createResourceListWidget(wantResources, bankResources, null);
        
        giveResourcesPickerWidget = createResourcePickerWidget(
                giveResources,  gamePanel.getPlayingPlayer().getPorts(), playerHand, gamePanel);
        
        wantedResourcesPickerWidget = createResourcePickerWidget(
                wantResources, null, bankResources, gamePanel);
        ...
        giveResources.addResourcesChangedEventHandler(new ResourcesChangedEventHandler()
        {
            @Override
        ...
        wantResources.addResourcesChangedEventHandler(new ResourcesChangedEventHandler()
        {
            @Override
            public void onResourcesChanged(ResourcesChangedEvent resourcesChanged)
            {
    ...
    {
        // Player can trade when amount of gold resources equals amount of picked resources
        btnTrade.setEnabled(
                gamePanel.getPlayingPlayer().amountGold(giveResources) == wantResources.size() &&
                wantResources.size() > 0);
    }
    private IResourceListWidget createResourceListWidget(ResourceList resources,ResourceList bankResources, PortList ports)
    {
    ...
    private IResourcePickerWidget createResourcePickerWidget
        (ResourceList resources, PortList ports,ResourceList bankResources, IGamePanel gamePanel)
    {
        return new BitmapResourcePickerWidget(resources, ports, bankResources, gamePanel);
    }
    ...
    public void setPieceToTradeFor(PlayerPiece pieceToTradeFor)
    {
        ...
        // When trading for a particular piece, the player cannot choose resources to trade for.
        ...
        if (pieceToTradeFor != null)
        {
            ...
            wantResources.add(playerHand.getNeededResources(pieceToTradeFor.getCost()));
            
            // Update image location
            imgPiece.setUrl(ImageLibrary.getIcon(pieceToTradeFor, 48));
    ...
    public TradeBankDialog()
    {
        ...
        imgPiece = new Image("icons/48/City48.png");
