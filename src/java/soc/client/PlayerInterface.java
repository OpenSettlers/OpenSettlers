...
/**
 * Window with interface for a player in one game of Settlers of Catan. Contains
 * {@link BoardPanel board}, client's and other players' {@link HandPanel hands}
 * , chat interface, game message window, and the {@link BuildingPanel
 * building/buying panel}.
 *<P>
 * Players' {@link HandPanel hands} start with player 0 at top-left, and go
 * clockwise; see {@link #doLayout()} for details.
 *<P>
 * When we join a game, the client will update visible game state by calling
 * methods here like {@link #addPlayer(String, int)}; when all this activity is
 * complete, and the interface is ready for interaction, the client calls
 * {@link #began()}.
 *<P>
 * A separate {@link PlayerClient} window holds the list of current games and
 * channels.
 * 
 * @author Robert S. Thomas
 */
public class PlayerInterface extends Frame implements ActionListener,
        MouseListener
{
     ...
     * @since 1.1.08
     */
    private final static boolean SOCPI_isPlatformWindows = (SOCPI_osName != null)
            && (SOCPI_osName.toLowerCase().indexOf("windows") != -1);
     ...
     * 
     * @see BoardPanel#isScaled()
     */
    protected boolean boardIsScaled;

    /**
     * Is this game using the 6-player board? Checks {@link Game#maxPlayers}.
     * 
     * @since 1.1.08
     */
    ...
    /**
     * For perf/display-bugs during component layout (OSX firefox), show only
     * background color in {@link #update(Graphics)} when true.
     * 
     * @since 1.1.06
     */
    ...
    /**
     * At least one text chat line has been sent by the player. Don't show the
     * initial prompt message if the text field becomes blank again.
     * 
     * @see #textInput
     * @see #TEXTINPUT_INITIAL_PROMPT_MSG
     */
    ...
    /**
     * Number of change-of-turns during game, after which the initial prompt
     * message fades to light grey.
     * 
     * @see #textInput
     ...
     * @see #textInput
     */
    public static final String TEXTINPUT_INITIAL_PROMPT_MSG = "Type here to chat.";

    /** Titlebar text for game in progress */
    public static final String TITLEBAR_GAME = "Settlers of Catan Game: ";

    /** Titlebar text for game when over */
    public static final String TITLEBAR_GAME_OVER = "Settlers of Catan Game Over: ";

    /**
     * Used for responding to textfield changes by setting/clearing prompt
     * message.
     * 
     * @see #textInput
     */
    ...
    /**
     * where text is displayed. In the 6-player layout, size expands when
    ...
    /**
     * where chat text is displayed. In the 6-player layout, size expands when
     * hovered over with mouse.
     * 
     * @see #textDisplaysLargerTemp
     */
     ...
     * {@link #textDisplay}, {@link #chatDisplay}) aren't as large. When this
     * flag is set, they've temporarily been made larger.
     * 
    ...
    /**
     * When set, must return text display field sizes to normal in
     * {@link #doLayout()} after a previous {@link #textDisplaysLargerTemp} flag
     * set.
     * 
     * @since 1.1.08
     */
    ...
    /**
     * Mouse hover flags, for use on 6-player board with
     * {@link #textDisplaysLargerTemp}.
     *<P>
     * Set/cleared in {@link #mouseEntered(MouseEvent)},
     * {@link #mouseExited(MouseEvent)}.
     * 
     * @see SOCPITextDisplaysLargerTask
     * @since 1.1.08
     */
    private boolean textInputHasMouse, textDisplayHasMouse,
            chatDisplayHasMouse;

    /**
     * In 6-player games, text areas temporarily zoom when the mouse is over
     * them. On windows, the scrollbars aren't considered part of the text
     * areas, so we get a mouseExited when user is trying to scroll the text
     * area. Workaround: Instead of looking for mouseExited, look for
     * mouseEntered on handpanels or boardpanel.
     * 
     * @see #textDisplaysLargerTemp
     * @see #sbFixBHasMouse
     */
    ...
    /**
     * Mouse hover flags, for use on 6-player board with
     * {@link #textDisplaysLargerTemp} and {@link #sbFixNeeded}. Used only on
     * platforms (windows) where the scrollbar isn't considered part of the
     * textarea and triggers a mouseExited.
     *<P>
     * Set/cleared in {@link #mouseEntered(MouseEvent)},
     * {@link #mouseExited(MouseEvent)}.
     * 
    ...
    /**
     * the display for the players' hands. Hands start at top-left and go
     * clockwise.
     */
    protected HandPanel[] hands;

    /**
     * Tracks our own hand within hands[], if we are active in a game. Null
     * otherwise. Set by HandPanel's removePlayer() and addPlayer() methods.
     */
    protected HandPanel clientHand;

    /**
     * Player ID of clientHand, or -1. Set by HandPanel's removePlayer() and
     * addPlayer() methods.
     */
    private int clientHandPlayerNum;

    /**
     * the player colors. Indexes from 0 to {@link Game#maxPlayers} - 1.
    ...
    /**
     * Flag to ensure interface is updated, when the first actual turn begins
     * (state changes from {@link Game#START2B} to {@link Game#PLAY}). Initially
     * set in {@link #startGame()}. Checked/cleared in
     * {@link #updateAtGameState()};
     */
    protected boolean gameIsStarting;

    /**
     * this other player has requested a board reset; voting is under way. Null
     * if no board reset vote is under way.
     * 
     * @see soc.server.SOCServer#resetBoardAndNotify(String, int)
     */
    protected HandPanel boardResetRequester;

    /**
     * Board reset voting: If voting is active and we haven't yet voted, track
     * our dialog; this lets us dispose of it if voting is cancelled.
     */
    protected ResetBoardVoteDialog boardResetVoteDia;

    /**
     * Is one or more handpanel (of other players) showing a "Discarding..."
     * message?
     */
    private boolean showingPlayerDiscards;

    /**
     * Synchronize access to {@link #showingPlayerDiscards} and
     * {@link #showingPlayerDiscardsTask}
     */
    ...
    /**
     * To reduce text clutter: server has just sent a dice result message. If
     * the next text message from server is the roll, replace: * It's Player's
     * turn to roll the dice. \n * Player rolled a 4 and a 5. with: * It's
     * Player's turn to roll. Rolled a 9.
     *<P>
     * Set to 0 at most times. Set to the roll result when roll text is
     * expected. Will be cleared to 0 in {@link #print(String)}. Whenever this
     * field is nonzero, textmessages from the server will be scanned for
     * " rolled a ".
     */
     ...
     */
    protected DiscardDialog discardDialog;

    /**
     * the dialog for choosing a player from which to steal
     */
    protected ChoosePlayerDialog choosePlayerDialog;

    /**
     * the dialog for choosing 2 resources to discover
     */
    protected DiscoveryDialog discoveryDialog;

    /**
     * the dialog for choosing a resource to monopolize
     */
    protected MonopolyDialog monopolyDialog;
     ...
     * 
     * @param title
     *            title for this interface - game name
     * @param cl
     *            the player client that spawned us
     * @param ga
     *            the game associated with this interface
     */
    public PlayerInterface(String title, PlayerClient cl, Game ga)
    {
        ...
        if (is6player)
        {
            playerColors[4] = new Color(97, 151, 113); // same green as
                                                       // playerclient bg
                                                       // ("61AF71")
        ...
        /**
         * setup interface elements. PERF: hide window while doing so (osx
         * firefox)
         */
        ...
        if (is6player)
            setSize((2 * HandPanel.WIDTH_MIN) + 16
                    + boardPanel.getMinimumSize().width, piHeight);
        else
         ...
         * set in PlayerClient.startPracticeGame or .guardedActionPerform).
         */
    }
     ...
     * 
     * @param firstCall
     *            First setup call for this window; do global things such as
     *            windowListeners, not just component-specific things.
     */
    protected void initInterfaceElements(boolean firstCall)
    {
         ...
         * Moved first so they'll be at top of the z-order, for use with
         * textDisplaysLargerTemp. In 6-player games, these text areas' sizes
         * are "zoomed" larger temporarily when the mouse hovers over them, for
         * better visibility.
         */
        ...
        textDisplay = new SnippingTextArea("", 40, 80,
                TextArea.SCROLLBARS_VERTICAL_ONLY, 80);
        ...
        chatDisplay = new SnippingTextArea("", 40, 80,
                TextArea.SCROLLBARS_VERTICAL_ONLY, 100);
        ...
        textInput.setSize(BoardPanel.PANELX, fm.getHeight() + 4);
         ...
         */
        hands = new HandPanel[game.maxPlayers];

        for (int i = 0; i < hands.length; i++)
        {
            HandPanel hp = new HandPanel(this, game.getPlayer(i));
            hands[i] = hp;
            hp.setSize(180, 120);
            add(hp);
        ...
        buildingPanel.setSize(200, BuildingPanel.MINHEIGHT);
        add(buildingPanel);
        ...
        {
            updatePlayerLimitDisplay(true, -1); // Player data may not be
                                                // received yet;
            // game is created empty, then SITDOWN messages are received from
            // server.
        }
        ...
        /**
         * If player requests window close, ask if they're sure, leave game if
         * so
         */
        if (firstCall)
        {
     ...
     * this directly, use {@link #repaint()} instead. For performance and
     * display-bug avoidance, checks {@link #layoutNotReadyYet} flag.
     */
     ...
     * @see #getClientHand()
     */
    public HandPanel getPlayerHandPanel(int pn)
    {
        return hands[pn];
    }
     ...
     * @see BoardPanel#popupSetBuildRequest(int, int)
     */
    ...
    /**
     * The game's count of development cards remaining has changed. Update the
     * display.
     */
    ...
    /**
     * The game's longest road or largest army may have changed. Update each
     * player's handpanel (victory points and longest-road/ largest-army
     * indicator). If it changed, print an announcement in the message window.
     *<P>
     * Call this only after updating the Game objects.
     * 
     * @param isRoadNotArmy
     *            Longest-road, not largest-army, has just changed
     * @param oldp
     *            Previous player with longest/largest, or null if none
     * @param newp
     *            New player with longest/largest, or null if none
     */
    public void updateLongestLargest(boolean isRoadNotArmy, Player oldp,
            Player newp)
    {
        // Update handpanels
        final int updateType;
        if (isRoadNotArmy)
            updateType = HandPanel.LONGESTROAD;
        else
            updateType = HandPanel.LARGESTARMY;

        for (int i = 0; i < game.maxPlayers; i++)
        {
            hands[i].updateValue(updateType);
            hands[i].updateValue(HandPanel.VICTORYPOINTS);
        }
    ...
    /**
     * Show the maximum and available number of player positions, if game
     * parameter "PL" is less than {@link Game#maxPlayers}. Also, if show, and
     * gamestate is {@link Game#NEW}, check for game-is-full, and hide or show
     * "sit down" buttons if necessary. If the game has already started, and the
     * client is playing in this game, will not show this display (it overlays
     * the board, which is in use). It will still hide/show sit-here buttons if
     * needed.
     * 
     * @param show
     *            show the text, or clear the display (at game start)?
     * @param playerLeaving
     *            The player number if a player is leaving the game, otherwise
     *            -1.
     * @since 1.1.07
     */
    private void updatePlayerLimitDisplay(final boolean show,
            final int playerLeaving)
    {
        final int gstate = game.getGameState();
        final boolean clientSatAlready = (clientHand != null);
        boolean noTextOverlay = ((!show) || ((gstate >= Game.START1A) && clientSatAlready));
        if (noTextOverlay)
        {
        ...
        if (!noTextOverlay)
        {
            String availTxt = (availPl == 1) ? "1 seat available" : Integer
                    .toString(availPl)
                    + " seats available";
            boardPanel.setSuperimposedText("Maximum players: " + maxPl,
                    availTxt);
        }
        if ((gstate == Game.NEW) || !clientSatAlready)
        {
            if (availPl == 0)
            {
    ...
    /**
     * The client player's HandPanel interface, if active in a game.
     * 
     * @return our player's hand interface, or null if not in a game.
     * @see #clientIsCurrentPlayer()
     */
    public HandPanel getClientHand()
    {
        return clientHand;
    }

    /**
     * Update the client player's HandPanel interface, for joining or leaving a
     * game.
     * 
     * Set by HandPanel's removePlayer() and addPlayer() methods.
     * 
     * @param h
     *            The HandPanel for us, or null if none (leaving).
     */
    public void setClientHand(HandPanel h)
    {
        clientHand = h;
        if (h != null)
     ...
     * Assertion: If this returns true, {@link #getClientHand()} will return
     * non-null.
     * 
     ...
     *<P>
     * Set to 0 at most times. Set to the roll result when roll text is
     * expected. Will be cleared to 0 in {@link #print(String)}.
     *<P>
     * Whenever this field is nonzero, textmessages from the server will be
     * scanned for " rolled a ".
     * 
     * @param roll
     *            The expected roll result, or 0.
     */
    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getSource() == textInput)
        {
            ...
            {
                // Player hit enter while chat prompt is showing
                ...
                textInput.setText(" "); // Not completely empty, so TextListener
                                        // won't re-set prompt.
                return;
            }
     ...
     * layout). If acceptable, send request to server. If not, say so in text
     * area. Not acceptable if they've already done so this turn, or if voting
    ...
    public void resetBoardRequest()
    {
        ...
        {
            textDisplay
                    .append("*** This server does not support board reset, server is too old.\n");
            return;
        }
        if (game.getResetVoteActive())
        {
            textDisplay
                    .append("*** Voting is already active. Try again when voting completes.\n");
            return;
        }
        Player pl = game.getPlayer(clientHandPlayerNum);
        if (!pl.hasAskedBoardReset())
            client.resetBoardRequest(game);
        else
            textDisplay
                    .append("*** You may ask only once per turn to reset the board.\n");
    }
    ...
    public void resetBoardVoted(int pn, boolean vyes)
    {
        ...
        if (vyes)
            ...
            voteMsg = "No thanks.";
        textDisplay.append("* " + game.getPlayer(pn).getName() + " has voted: "
                + voteMsg + "\n");
    ...
    /**
     * Voting complete, board reset was rejected. Display text message and clear
     * the offer.
     */
    public void resetBoardRejected()
    {
        ...
        for (int i = 0; i < hands.length; ++i)
        {
            ...
            try
            {
                hands[i].resetBoardSetMessage(null);
            }
            catch (IllegalStateException e)
            { /* ignore; discard message is showing */
            }
        ...
        // Requester may have already been null, if we're the requester and it
        // was rejected.
    }

    /**
     * Creates and shows a new ResetBoardVoteDialog. If the game is over, the
     * "Reset" button is the default; otherwise, "No" is default. Also announces
     * the vote request (text) and sets boardResetRequester. Dialog is shown in
     * a separate thread, to continue message treating and screen redraws as the
     * other players vote.
     *<P>
     * If we are the requester, we update local game state but don't vote.
     * 
     * @param pnRequester
     *            Player number of the player requesting the board reset
     */
    public void resetBoardAskVote(int pnRequester)
    {
        boolean gaOver = (game.getGameState() >= Game.OVER);
        try
        {
        ...
        if (pnRequester != clientHandPlayerNum)
        {
            ...
            boardResetVoteDia = new ResetBoardVoteDialog(client, this,
                    requester, gaOver);
    ...
    /**
     * Callback from ResetBoardVoteDialog, to clear our reference when button is
     * clicked and dialog is going away.
     */
    ...
    /**
     * print text in the text window. For dice-roll message, combine lines to
     * reduce clutter.
     * 
     * @param s
     *            the text
    ...
    public void print(String s)
    {
        ...
        {
            /*
             * Special case: Roll message. Reduce clutter. Instead of printing
             * this message verbatim, change the textDisplay contents (if
             * matching): replace: * It's Player's turn to roll the dice. \n *
             * Player rolled a 4 and a 5. with: * It's Player's turn to roll.
             * Rolled a 9.
             * 
             * JM 2009-05-21: Don't edit existing text on Mac OS X 10.5; it can
             * lead to a GUI hang/race condition. Instead just print the total
             * rolled.
             */
        ...
        while (st.hasMoreElements())
        {
            ...
            textDisplay.append(tk + "\n"); // TextArea will soft-wrap within the
                                           // line
        }
    }
     ...
     * This doesn't mean that game play or placement is starting, only that the
     * window is ready for players to choose where to sit. By now HandPanel has
     * added "sit" buttons, or updatePlayerLimitDisplay has removed them if
     * necessary.
     */
    public void began()
    {
    ...
    {
        hands[pn].addPlayer(n); // This will also update all other hands'
                                // buttons ("sit here" -> "lock", etc)
    ...
    /**
     * remove a player from the game. To prevent inconsistencies, call this
     * <em>before</em> calling {@link Game#removePlayer(String)}.
     * 
     * @param pn
    ...
    public void removePlayer(int pn)
    {
        ...
        if (game.isGameOptionDefined("PL"))
            ...
            hands[pn].addSitButton(clientHand != null); // Is the client player
    ...
    /**
     * Game play is starting (leaving state {@link Game#NEW}). Remove the start
     * buttons and robot-lockout buttons. Next move is for players to make their
     * starting placements.
     */
    public void startGame()
    {
        for (int i = 0; i < hands.length; i++)
        {
            ...
            // If client joined and then started a game, remove it (as robot
            // lockout).
     ...
     * 
     * @param finalScores
     *            Final score for each player position
     */
    public void updateAtOver(int[] finalScores)
    {
        ...
        {
            game.checkForWinner(); // Assumes "current player" set to winner
                                   // already, by SETTURN msg
        }
        for (int i = 0; i < finalScores.length; ++i)
            hands[i].updateValue(HandPanel.VICTORYPOINTS); // Also disables
                                                           // buttons, etc.
     ...
     * 
     * @param pnum
     *            New current player number; should match
    ...
    /**
     * Set or clear the chat text input's initial prompt. Sets its status,
     * foreground color, and the prompt text if true.
     * 
     * @param setToInitial
     *            If false, clear initial-prompt status, and clear contents (if
     *            they are the initial-prompt message); If true, set
     *            initial-prompt status, and set the prompt (if contents are
     *            blank when trimmed).
     * 
     * @throws IllegalStateException
     *             if setInitial true, but player already sent chat text
     *             (textInputHasSent).
     * 
     * @see #TEXTINPUT_INITIAL_PROMPT_MSG
     */
            ...
            throws IllegalStateException
    {
        if (setToInitial && textInputHasSent)
            throw new IllegalStateException(
                    "Already sent text, can't re-initial");
        ...
        if (setToInitial)
        {
            if (textInput.getText().trim().length() == 0)
                textInput.setText(TEXTINPUT_INITIAL_PROMPT_MSG); // Set text
                                                                 // before flag
            textInputIsInitial = true;
            textInputGreyCountdown = textInputGreyCountFrom; // Reset fade
                                                             // countdown
        ...
        else
        {
            if (textInput.getText().equals(TEXTINPUT_INITIAL_PROMPT_MSG))
                textInput.setText(""); // Clear to make room for text being
                                       // typed
    ...
    {
        discardDialog = new DiscardDialog(this, nd);
     ...
     * 
     * @param count
     *            the number of players to choose from
     * @param pnums
     *            the player ids of those players
     */
    public void choosePlayer(int count, int[] pnums)
    {
        choosePlayerDialog = new ChoosePlayerDialog(this, count, pnums);
    ...
    {
        discoveryDialog = new DiscoveryDialog(this);
    ...
    /**
     * Update interface after game state has changed. Please call after
     * {@link Game#setGameState(int)}. If the game is now starting, please call
     * in this order: <code>
    ...
    public void updateAtGameState()
    {
        ...
        if ((gs == Game.PLACING_ROAD) || (gs == Game.PLACING_SETTLEMENT)
                || (gs == Game.PLACING_CITY))
        {
        ...
        if (gameIsStarting && (gs >= Game.PLAY))
        {
            gameIsStarting = false;
            if (clientHand != null)
        ...
        if (clientHand != null)
        {
            Player ourPlayerData = clientHand.getPlayer();
            if (ourPlayerData.getPlayerNumber() == game
                    .getCurrentPlayerNumber())
            {
                if (gs == Game.WAITING_FOR_DISCOVERY)
                {
                    showDiscoveryDialog();
                }
                else if (gs == Game.WAITING_FOR_MONOPOLY)
                {
                    showMonopolyDialog();
                }
                else if (gs == Game.PLAY1)
                {
                    updateAtPlay1();
                }
            }
    ...
    /**
     * Gamestate just became {@link Game#WAITING_FOR_DISCARDS}. Set up a timer
     * to wait 1 second before showing "Discarding..." balloons in players'
     * handpanels.
     */
    private void discardTimerSet()
    {
        synchronized (showingPlayerDiscards_lock)
        {
            ...
            client.getEventTimer()
                    .schedule(showingPlayerDiscardsTask, 1000 /* ms */);
        }
    }
    ...
    /**
     * Handle board reset (new game with same players, same game name). The
     * reset message will be followed with others which will fill in the game
     * state.
     * 
     * @param newGame
     *            New game object
     * @param rejoinPlayerNumber
     *            Sanity check - must be our correct player number in this game
     * @param requesterNumber
     *            Player who requested the board reset
     * 
     * @see soc.server.SOCServer#resetBoardAndNotify(String, int)
     */
    public void resetBoard(Game newGame, int rejoinPlayerNumber,
            int requesterNumber)
    {
        if (clientHand == null)
            return;
        ...
        // Feedback: "busy" mouse cursor while clearing and re-laying out the
        // components
        ...
        for (int i = 0; i < hands.length; ++i)
        {
            hands[i].removePlayer(); // will cancel roll countdown timer,
    ...
    /**
     * if debug is enabled, print this exception's stack trace in the chat
     * display. This eases tracing of exceptions when our code is called in AWT
     * threads (such as EventDispatch).
     */
    ...
    /**
     * Calculate a color towards gray, for a hilight or the robber ghost. If
     * srcColor is light, ghost color is darker. (average with gray) If it's
     * dark or midtone, ghost should be lighter. (average with white)
     * 
     * @param srcColor
    ...
    /**
     * Arrange the custom layout. If a player sits down in a 6-player game, will
     * need to {@link #invalidate()} and call this again, because
     * {@link HandPanel} sizes will change. Also, on first call, resets mouse
     * cursor to normal, in case it was WAIT_CURSOR.
     */
    public void doLayout()
    {
        ...
        /**
         * Classic Sizing (board size is fixed, cannot scale)
         * 
         * int bw = BoardPanel.getPanelX(); int bh = BoardPanel.getPanelY(); int
         * hw = (dim.width - bw - 16) / 2; int hh = (dim.height - 12) / 2; int
         * kw = bw; int kh = buildingPanel.getSize().height; int tfh =
         * textInput.getSize().height; int tah = dim.height - bh - kh - tfh -
         * 16;
         */

        /**
         * "Stretch" Scaleable-board Sizing: If board can be at least 15% larger
         * than minimum board width, without violating minimum handpanel width,
         * scale it larger. Otherwise, use minimum board width (widen handpanels
         * instead). Handpanel height: - If 4-player, 1/2 of window height - If
         * 6-player, 1/3 of window height, until client sits down. (Column of 3
         * on left side, 3 on right side, of this frame) Once sits down, that
         * column's handpanel heights are 1/2 of window height for the player's
         * hand, and 1/4 for others.
         */
        ...
        int bw = (dim.width - 16 - (2 * HandPanel.WIDTH_MIN)); // As wide as
                                                               // possible
        ...
        if (bh > (dim.height - buildph - 16 - (int) (5.5f * tfh)))
        {
            ...
            bh = dim.height - buildph - 16 - (int) (5.5f * tfh); // As tall as
                                                                 // possible
        ...
        if (canScaleBoard)
        {
            try
            {
                boardPanel.setBounds(i.left + hw + 8, i.top + tfh + tah + 8,
                        bw, bh);
            }
        ...
        try
        {
            if (!is6player)
            {
                ...
                {
                    hands[1]
                            .setBounds(i.left + hw + bw + 12, i.top + 4, hw, hh);
                    hands[2].setBounds(i.left + hw + bw + 12, i.top + hh + 8,
                            hw, hh);
            ...
            {
                // 6-player layout:
                // If client player isn't sitting yet, all handpanels are 1/3
                // height of window.
                // Otherwise, they're 1/3 height in the column of 3 which
                // doesn't contain the
                ...
                if ((clientHandPlayerNum < 1) || (clientHandPlayerNum > 3))
                {
                    ...
                    hands[2].setBounds(i.left + hw + bw + 12, i.top + hh + 8,
                            hw, hh);
                    hands[3].setBounds(i.left + hw + bw + 12, i.top + 2 * hh
                            + 12, hw, hh);
                }
                if (clientHandPlayerNum != -1)
                {
        ...
        /**
         * Reset mouse cursor from WAIT_CURSOR to normal (set in
         * PlayerClient.startPracticeGame or .guardedActionPerform).
         */
        if (layoutNotReadyYet)
        {
     ...
     * enters/exits them. Timer will set textDisplaysLargerTemp_needsLayout,
     * invalidate(), doLayout(). Wait 200 ms first, to avoid flicker in case of
     * several related {@link #mouseExited(MouseEvent)}/
     * {@link #mouseEntered(MouseEvent)} events (such as moving mouse from
     * {@link #textDisplay} to {@link #chatDisplay}).
     * 
     * @since 1.1.08
     */
    ...
    {
        getEventTimer()
                .schedule(new SOCPITextDisplaysLargerTask(), 200 /* ms */);
    }

    /**
     * Track TextField mouse cursor entry on 6-player board (MouseListener).
     * Listener is not added unless {@link #is6player}. For use by
     * {@link #textDisplay}, {@link #chatDisplay}, {@link #textInput}. Calls
     * {@link #textDisplaysLargerSetResizeTimer()}.
     * 
     * @since 1.1.08
     */
    ...
    /**
     * Track TextField mouse cursor exit on 6-player board (MouseListener). Same
     * details as {@link #mouseEntered(MouseEvent)}.
     * 
     * @since 1.1.08
     */
    ...
    /**
     * This is the dialog to vote on another player's board reset request. If
     * game in progress, buttons are Reset and Continue Playing; default
     * Continue. If game is over, buttons are Restart and No thanks; default
     * Restart. Start a new thread to show, so message treating can continue as
     * other players vote.
     * 
     * @author Jeremy D Monin <jeremy@nand.net>
     */
    protected static class ResetBoardVoteDialog extends AskDialog implements
            Runnable
    {
        /**
         * 
         ...
         * @param gameIsOver
         *            The game is over - "Reset" button should be default (if
         *            not over, "Continue" is default)
         */
        protected ResetBoardVoteDialog(PlayerClient cli,
                PlayerInterface gamePI, String requester, boolean gameIsOver)
        {
            super(
                    cli,
                    gamePI,
                    "Reset board of game " + gamePI.getGame().getName() + "?",
                    (gameIsOver ? (requester + " wants to start a new game.")
                            : (requester + " wants to reset the game being played.")),
                    (gameIsOver ? "Restart" : "Reset"),
                    (gameIsOver ? "No thanks" : "Continue playing"), null,
                    (gameIsOver ? 1 : 2));
            rdt = null;
        ...
        {
            pcli
                    .resetBoardVote(pi.getGame(), pi.getClientPlayerNumber(),
                            false);
            pi.resetBoardClearDia();
        }
        ...
        /**
         * Make a new thread and show() in that thread. Keep track of the
         * thread, in case we need to dispose of it.
         */
        ...
        /**
         * In new thread, show ourselves. Do not call directly; call
         * {@link #showInNewThread()}.
         */
        public void run()
        {
            try
    ...
    private static class MyWindowAdapter extends WindowAdapter
    {
        ...
        public MyWindowAdapter(PlayerInterface spi)
        {
            pi = spi;
        }
        ...
        {
            // leaveGame();
            QuitConfirmDialog.createAndShow(pi.getClient(), pi);
        }
    } // MyWindowAdapter

    /**
     * Used for chat textfield setting/clearing initial prompt text
     * (TEXTINPUT_INITIAL_PROMPT_MSG). It's expected that after the player sends
     * their first line of chat text, the listeners will be removed so we don't
     * have the overhead of calling these methods.
     */
    private static class SOCPITextfieldListener extends KeyAdapter implements
            TextListener, FocusListener
    {
        private PlayerInterface pi;

        public SOCPITextfieldListener(PlayerInterface spi)
        {
            pi = spi;
        }
        ...
        /**
         * If input text is cleared, and field is again empty, show the prompt
         * message unless player has already sent a line of chat.
         */
        ...
        /**
         * If input text is cleared, and player leaves the textfield while it's
         * empty, show the prompt message unless they've already sent a line of
         * chat.
         */
        ...
        /**
         * Clear the initial prompt message when textfield is entered or clicked
         * on.
         */
    ...
    /**
     * When timer fires, show discard message in any other player (not client
     * player) who must discard.
     * 
     * @see PlayerInterface#discardTimerSet()
     */
    private static class SOCPIDiscardMsgTask extends TimerTask
    {
        private PlayerInterface pi;

        public SOCPIDiscardMsgTask(PlayerInterface spi)
        {
            pi = spi;
        }

        /**
         * Called when timer fires. Examine game state and players. Sets
         * "discarding..." to handpanels of discarding players.
         */
        public void run()
        {
            ...
            {
                if (pi.game.getGameState() != Game.WAITING_FOR_DISCARDS)
                {
                ...
                pi.showingPlayerDiscardsTask = null; // No longer needed (fires
                                                     // once)
            }
        }
    ...
    /**
     * For 6-player board, make the text displays larger/smaller when mouse
     * enters/exits them. Wait 200 ms first, to avoid flicker in case of several
     * related {@link #mouseExited(MouseEvent)}/
     * {@link #mouseEntered(MouseEvent)} events (such as moving mouse from
     * {@link #textDisplay} to {@link #chatDisplay}).
     *<P>
     * Used only when {@link PlayerInterface#is6player} true.
     *<P>
    ...
    private class SOCPITextDisplaysLargerTask extends TimerTask
    {
        ...
        public void run()
        {
            final boolean leftLarger = sbFixNeeded
                    && (sbFixLHasMouse || sbFixRHasMouse || sbFixBHasMouse);
            final boolean wantsLarger = (textDisplayHasMouse
                    || chatDisplayHasMouse || textInputHasMouse)
    ...
    private class InputKeyListener extends KeyAdapter
    {
        public void keyPressed(KeyEvent e)
        {
            ...
            if ((key == KeyEvent.VK_UP) && (hs > historyCounter))
            {
                ...
                historyCounter++;
                textInput.setText((String) history.elementAt(hs
                        - historyCounter));
            }
